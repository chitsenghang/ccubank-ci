...
{
  listenTo(): typeof AuditBaseEntity {
    return AuditBaseEntity;
  }

  async afterInsert(event: InsertEvent<AuditBaseEntity>): Promise<void> {
  ...
  async afterUpdate(event: UpdateEvent<AuditBaseEntity>): Promise<void> {
    const { entity, manager } = event;
